...
def payment_success(request, pk):
    """
    Unified success view — works for both:
      - Tour Proposal → Booking (via proposal_id)
      - Accommodation → AccommodationBooking (via booking_id)
    """
    # Try to find by Proposal → Booking
    proposal = Proposal.objects.filter(id=pk).first()
    if proposal and proposal.status == 'PAID':
        booking = Booking.objects.filter(proposal=proposal).first()
        if booking:
            context = {
                'type': 'tour',
                'booking': booking,
    ...
    # Try to find by AccommodationBooking
    acc_booking = get_object_or_404(AccommodationBooking, id=pk)

    if acc_booking.status != 'PAID':
        # Fallback: mark as paid if PayPal said so
        if acc_booking.status == 'PENDING_PAYMENT':
