document.addEventListener('DOMContentLoaded', function () {
    ...
    if (currencyForm && currencyToggle && currencyDropdown && currencyInput) {
        ...
        function configureCurrencyForm(bookingForm) {
            if (!bookingForm) {
                console.warn('No booking form found, hiding currency toggle');
            ...
            if (tourType && tourId) {
                ...
                currencyForm.setAttribute('hx-target', '#pricing-options');
                currencyForm.setAttribute('hx-swap', 'innerHTML');
                console.log('Currency form configured:', { 'hx-post': url, 'hx-target': '#pricing-options', 'hx-swap': 'innerHTML' });
                const pricingOptions = document.querySelector('#pricing-options');
                if (pricingOptions) {
                    console.log('Target #pricing-options found:', pricingOptions);
                } else {
                    console.error('Target #pricing-options not found in DOM. HTMX will fail.');
                }
            } else {
                console.warn('Cannot set currency form hx-post: missing tourType or tourId', { tourType, tourId });
                currencyToggle.classList.add('hidden');
            }
        }
        ...
        currencyDropdown.addEventListener('click', function (event) {
            ...
            if (currencyButton) {
                ...
                if (bookingForm) {
                    ...
                    console.warn('Booking form not found, sending minimal data');
                }
    ...
    window.updateSelectedConfiguration = function () {
        ...
        if (selectedRadio && selectedConfigInput) {
            ...
            if (selectedConfigInput) selectedConfigInput.value = '0';
        }
    };
    ...
    window.initializePricingControls = function (pricingTable) {
        ...
        // CHANGED: Reset initialized state to ensure re-initialization post-HTMX
        ...
        const clearFiltersButton = pricingTable.querySelector('#clear-filters'); // CHANGED: Query within pricingTable
        ...
        // ADDED: Restore filter state from localStorage
        ...
        applyFilters(pricingTable); // CHANGED: Apply filters on init to reflect saved state
    };
    ...
    function applyFilters(pricingTable) {
        ...
        // ADDED: Save filter state to localStorage
        ...
        const filteredItems = Array.from(configurationItems).filter(item => {
            ...
            // CHANGED: Robust price parsing to handle commas
    ...
    function clearFilters(pricingTable) {
        ...
        // ADDED: Clear saved filter state
        localStorage.setItem('pricingFilters', JSON.stringify({
            roomTypes: [],
            priceMin: '',
            priceMax: ''
    ...
    document.addEventListener('htmx:afterSwap', function (evt) {
        ...
        // Removed recursive updateChildAges call for child-ages-container
    });
    ...
    document.addEventListener('htmx:afterSettle', function (evt) {
        ...
        if (evt.detail.target.id === 'pricing-options') {
            ...
            if (pricingTable) {
                // CHANGED: Force re-initialization to ensure filter listeners
    ...
    function initializeBookingForm(modalId) {
        ...
        if (submitButton) {
            submitButton.addEventListener('click', function (event) {
                ...
                if (!isValid) {
                    if (errorMessage) {
                        errorMessage.textContent = errorMessage.textContent || 'Please fill out all required fields, including child ages.';
                        errorMessage.classList.remove('hidden');
            ...
            document.body.addEventListener('htmx:afterRequest', function (event) {
                if (event.detail.target.id === 'booking-form-container') {
                    const errorMessage = document.getElementById('form-error') || document.getElementById('htmx-error-message');
                    if (event.detail.successful) {
                        console.log('HTMX submission successful');
                        if (errorMessage) errorMessage.classList.add('hidden');
                    } else {
                        console.error('HTMX submission failed:', event.detail.xhr.status, event.detail.xhr.responseText);
                        if (errorMessage) {
                            errorMessage.textContent = 'Failed to submit proposal. Please try again.';
                            errorMessage.classList.remove('hidden');
                        }
                    }
                    if (submitButton) {
